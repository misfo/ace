    ...
    $buildKeymappingRegex: function(keymapping) {
        for (state in keymapping) {
            if (state === "onStateChange") {
              console.log("found onStateChange");
              this.$onStateChange = keymapping[state];
            } else {
    ...
    $find: function(data, buffer, symbolicName, hashId, key) {
        ...
        this.keymapping[data.state].some(function(binding) {
            ...
            if (binding.then) {
                ...
                data.buffer = "";
                if (onStateChange) {
                    var commandOnly = result.command;
                    if (typeof commandOnly === "string") {
                        result.command = {
                            exec: function(env, args, request) {
                                //FIXME this isn't executing
                                // allow passing multiple commands up so the exec doesn't happen here
                                var success = canon.exec(commandOnly.command, env, "editor", args);
                                onStateChange(env, {state: data.state}, request);
                                return success;
                            }
                        }
                    } else {
                        result.command = function(env, args, request) {
                            var success = commandOnly(env, args, request);
                            this.$onStateChange(env, {state: data.state}, request);
                            return success;
                        };
                    }
                }
